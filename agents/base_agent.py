        ...
        try:
            ...
            if isinstance(parsed, (AgentOutput, BootstrapOutput)):
                ...
                if step_domain is not None:
                    # Prefer passing through the provided structured dict context when available.
                    if isinstance(context, dict):
                        ctx_obj = context
                    else:
                        # Attempt to build a minimal structured context from the DB when
                        # the caller passed a plain string (common in the orchestrator
                        # contextual recommendations pass). This helps ROI enrichment
                        # detect property/attic metrics such as area and current R.
                        ctx_obj = {}
                        try:
                            if audit_id:
                                ...
                                if audit:
                                    # Audit-level ROI defaults (if present)
                                    try:
                                        rd = getattr(audit, "roi_defaults", {}) or {}
                                        if rd:
                                            if rd.get("energy_rate_usd_per_kwh") is not None:
                                                ctx_obj["energy_rate_usd_per_kwh"] = rd.get("energy_rate_usd_per_kwh")
                                            if rd.get("climate") is not None:
                                                ctx_obj["climate"] = rd.get("climate")
                                            if rd.get("analysis_horizon_years") is not None:
                                                ctx_obj["analysis_horizon_years"] = rd.get("analysis_horizon_years")
                                    except Exception:
                                        pass

                                    # Property sqft
                                    try:
                                        if getattr(audit, "property", None) and getattr(audit.property, "sqft", None):
                                            ctx_obj["property_sqft"] = audit.property.sqft
                                    except Exception:
                                        pass

                                    # Inspect insulation steps for ai_summary/meta hints
                                    try:
                                        steps = AuditStep.query.filter_by(audit_id=audit_id).all()
                                        # Look for attic-related ai_summary keys
                                        for s in steps:
                                            # ai_summary may contain numeric hints like current_r or thickness_inches
                                            if getattr(s, "ai_summary", None) and isinstance(s.ai_summary, dict):
                                                ai = s.ai_summary
                                                # direct current R
                                                if ai.get("current_r"):
                                                    try:
                                                        ctx_obj["attic_current_r"] = float(ai.get("current_r"))
                                                        break
                                                    except Exception:
                                                        pass
                                                # fallback: thickness_inches -> approximate R (approx 3 per inch)
                                                if ai.get("thickness_inches"):
                                                    try:
                                                        ctx_obj["attic_current_r"] = float(ai.get("thickness_inches")) * 3.0
                                                        break
                                                    except Exception:
                                                        pass

                                        # Try to extract attic area from step.meta if available
                                        for s in steps:
                                            if getattr(s, "meta", None) and isinstance(s.meta, dict):
                                                aarea = s.meta.get("attic_area_sqft") or s.meta.get("area_sqft")
                                                if aarea:
                                                    try:
                                                        ctx_obj["attic_area_sqft"] = float(aarea)
                                                        break
                                                    except Exception:
                                                        pass

                                        # Final fallback: derive attic_area from property sqft (35% heuristic)
                                        if "attic_area_sqft" not in ctx_obj and "property_sqft" in ctx_obj:
                                            try:
                                                ctx_obj["attic_area_sqft"] = round(float(ctx_obj["property_sqft"]) * 0.35, 2)
                                            except Exception:
                                                pass
                                    except Exception:
                                        # any failure here shouldn't break agent flow
                                        logger.debug("Failed to build structured ROI context from DB", exc_info=True)
                        except Exception:
                            logger.debug("Failed to build ctx_obj from DB", exc_info=True)
